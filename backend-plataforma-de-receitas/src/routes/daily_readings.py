...
def _norm(path):
    """Normaliza path para comparação (case-insensitive, barras uniformes)."""
...
def add_daily_reading():
    ...
    if any(_norm(r.get('path')) == _norm(path) for r in readings):
...
def remove_daily_reading():
    ...
    readings = _read_daily_readings()
    readings = [r for r in readings if _norm(r.get('path')) != _norm(path)]
    _write_daily_readings(readings)
