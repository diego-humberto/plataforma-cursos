      ...
      <Tooltip.Root>
        <Tooltip.Trigger asChild>
          <Menu.Button className={buttonClass}>
            <SettingsIcon className="h-5 w-5 transform transition-transform duration-200 ease-out group-data-[open]:rotate-90" />
          </Menu.Button>
        </Tooltip.Trigger>
...
const OdometerIconWrapper: React.FC<React.SVGProps<SVGSVGElement>> = (props) => {
  const { ref, ...restProps } = props;
  return <OdometerIcon {...restProps} />;
};

function SpeedSubmenu() {
  const options = usePlaybackRateOptions({
    rates: [0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 2.25, 2.5],
  }),
  ...
  return (
    <Menu.Root>
      <SubmenuButton
        ...
        hint={hint}
        disabled={options.disabled}
        Icon={OdometerIconWrapper}
      />
      <Menu.Content className={submenuClass}>
        <Menu.RadioGroup className="w-full flex flex-col" value={options.selectedValue}>
          {options.map(({ label, value, select }) => (
            <Radio value={value} onSelect={(e) => {
              select(e);
